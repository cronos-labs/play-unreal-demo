...
#include "CryptoFireProjectile.h"
...
//////////////////////////////////////////////////////////////////////////
// ACryptoFireCharacter

ACryptoFireCharacter::ACryptoFireCharacter() {
...
void ACryptoFireCharacter::BeginPlay() {
    // Call the base class
    Super::BeginPlay();
}
...
void ACryptoFireCharacter::SetupPlayerInputComponent(
    ...
    PlayerInputComponent->BindAction("PrimaryAction", IE_Pressed, this,
                                     &ACryptoFireCharacter::OnPrimaryAction);
    ...
    PlayerInputComponent->BindAxis("Move Forward / Backward", this,
                                   &ACryptoFireCharacter::MoveForward);
    PlayerInputComponent->BindAxis("Move Right / Left", this,
                                   &ACryptoFireCharacter::MoveRight);

    // We have 2 versions of the rotation bindings to handle different kinds of
    // devices differently "Mouse" versions handle devices that provide an
    // absolute delta, such as a mouse. "Gamepad" versions are for devices that
    // we choose to treat as a rate of change, such as an analog joystick
    ...
    PlayerInputComponent->BindAxis("Turn Right / Left Gamepad", this,
                                   &ACryptoFireCharacter::TurnAtRate);
    PlayerInputComponent->BindAxis("Look Up / Down Gamepad", this,
                                   &ACryptoFireCharacter::LookUpAtRate);
}

void ACryptoFireCharacter::OnPrimaryAction() {
    // Trigger the OnItemUsed Event
    OnUseItem.Broadcast();
}

void ACryptoFireCharacter::BeginTouch(const ETouchIndex::Type FingerIndex,
...
void ACryptoFireCharacter::EndTouch(const ETouchIndex::Type FingerIndex,
                                    const FVector Location) {
...
void ACryptoFireCharacter::MoveForward(float Value) {
    if (Value != 0.0f) {
...
void ACryptoFireCharacter::MoveRight(float Value) {
    if (Value != 0.0f) {
        // add movement in that direction
...
void ACryptoFireCharacter::TurnAtRate(float Rate) {
    // calculate delta for this frame from the rate information
    AddControllerYawInput(Rate * TurnRateGamepad *
                          GetWorld()->GetDeltaSeconds());
}

void ACryptoFireCharacter::LookUpAtRate(float Rate) {
    // calculate delta for this frame from the rate information
    AddControllerPitchInput(Rate * TurnRateGamepad *
                            GetWorld()->GetDeltaSeconds());
}

bool ACryptoFireCharacter::EnableTouchscreenMovement(
    class UInputComponent *PlayerInputComponent) {
    if (FPlatformMisc::SupportsTouchInput() ||
        ...
        PlayerInputComponent->BindTouch(EInputEvent::IE_Pressed, this,
                                        &ACryptoFireCharacter::BeginTouch);
        PlayerInputComponent->BindTouch(EInputEvent::IE_Released, this,
                                        &ACryptoFireCharacter::EndTouch);
